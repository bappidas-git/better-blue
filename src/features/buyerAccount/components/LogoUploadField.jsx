...
// The company logo control: a preview of what is on file, a picker that uploads
// the moment a file is chosen, and a way back to the generated initials mark.
//
// The field's value is the **logo URL**, not a `File`. `FormFileField` emits
...
/**
 * @param {object} props
 * @param {string} [props.value] the logo URL currently on the form
 * @param {(url: string) => void} props.onChange receives the new logo URL
 * @param {string} [props.companyName] name the initials fallback is generated from
...
export default function LogoUploadField({
  ...
  const initialsLogo = avatarDataUri(companyName || 'BetterBlue')
  const isGenerated = !value || value === initialsLogo
  const preview = value || initialsLogo
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2.5} alignItems="flex-start">
        <Box sx={{ position: 'relative', flexShrink: 0 }}>
          <Box
            ...
            alt={
              isGenerated
                ? `Generated initials logo for ${companyName || 'your business'}`
                : `Current logo for ${companyName || 'your business'}`
            }
            sx={{
              width: 96,
              height: 96,
          ...
          {isUploading ? (
            ...
            >
              <CircularProgress size={24} aria-label="Uploading your logo" />
            </Box>
          ) : null}
        </Box>
        ...
        <Stack spacing={1} sx={{ minWidth: 0, flexGrow: 1 }}>
          <Typography variant="body2" color="text.secondary">
            A square image works best — it appears beside your briefs and in every conversation
            with a creator. Without one, we use your initials.
          </Typography>
          <Box>
            <Button
              variant="text"
