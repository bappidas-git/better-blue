...
// per query key, and gets back sanitised values plus writers. Prompt 12 wrote
// this for creator discovery; Prompt 23 lifted it here unchanged so the request
// board could declare its own schema against the same machinery
// (`features/discovery/hooks/useDiscoveryParams.js` re-exports it under its
// original names, so Prompt 12's call sites did not move).
...
/**
 * Codecs for the query keys a list page declares. Each one is total: any
 ...
 *
 * @example
 * const { values, setValue, clearFilters } = useListParams(schema, { context })
 * <SearchInput value={values.q} onChange={(q) => setValue('q', q)} />
 */
export function useListParams(schema, { context = null, pageKey = 'page' } = {}) {
