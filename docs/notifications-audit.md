...
| Creator opens a reply on a feed | `feedService.createReply` | feed's buyer | `feed_reply_received` | ✅ **Storefront V2-03.** A reply is a message, not a priced offer, so it does not reuse `proposal_received` — telling a buyer they have an offer to review when what arrived is a question would send them to the wrong screen. Filed under the same `marketplace` category, so no new preference row appears. The notification links to the brief: the buyer-facing thread view is future work (V2-08 builds the creator's side), and the brief is the nearest true destination. |
| Creator adds a message to an existing reply thread | — | — | — | ➖ Not yet. The thread has no buyer-facing screen to send them to, so a notification per message would be a link to a page that cannot show it. Revisit when the buyer's side of the conversation is built. |
